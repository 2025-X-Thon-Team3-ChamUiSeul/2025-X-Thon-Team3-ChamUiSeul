...
export default function ChatPage() {
  ...
  const [isLoading, setIsLoading] = useState(false); // ⬅️ 로딩 상태 추적
  const [sessions, setSessions] = useState([]); // 기존 채팅 세션
  const [progressList, setProgressList] = useState([]); // 서류 진행 현황
  ...
  // 컴포넌트가 처음 로드될 때 URL에서 토큰을 추출하고 데이터 로딩
  useEffect(() => {
    ...
    if (token) {
      // 토큰과 유저 정보를 로컬 스토리지에 저장하여 로그인 상태 유지
      ...
      // URL에서 토큰 정보를 제거하여 주소를 깔끔하게 유지
      window.history.replaceState({}, document.title, "/chat");
    } else {
      // 새로고침 시 로컬 스토리지에서 사용자 이름 불러오기
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      const data = await response.json();
      const botMsg = { sender: "bot", text: data.response }; // ⬅️ API 응답 적용
      ...
      const errorMsg = {
        sender: "bot",
        text: "죄송해요, 응답을 생성하는 중에 오류가 발생했어요.",
      };
    ...
    >
      <div className="chat-container">
        {progressList && progressList.length > 0 && (
          <ProgressTracker progress={progressList[0]} />
        )}
        ...
        <div
          className={`input-bar ${messages.length === 0 ? "intro-bottom" : ""}`}
        >
          <input
            value={input}
